...
class UpConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, activation,
                 kernel_size=21, norm=True, pool=True):
        ...
        if pool:
            layers.append(nn.MaxPool2d((5,5), stride=1, padding=2))
...
class DownConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, activation, iteration,
                 kernel_size=21, norm=False):
...
class Generator(nn.Sequential):
    def __init__(self, n_z, input_filt=512, norm=False, pool=False,
                 n_layers=5, out_channels=3, final_size=512):
        ...
        initial_size = final_size // (2 ** n_layers)
        if final_size % (2 ** n_layers) != 0:
            raise ValueError(
                f"final_size must be divisible by 2^{n_layers}"
            )

        super().__init__(
            nn.Linear(n_z, initial_size * initial_size * input_filt),
            nn.LeakyReLU(0.2, True),
            Rearrange('b (h w z) -> b z h w',
                      h=initial_size, w=initial_size, z=input_filt),
            *layers,
            nn.Conv2d(prev_filt, out_channels, kernel_size=5, padding=2),
            nn.Sigmoid()
        )
...
class Discriminator(nn.Sequential):
    def __init__(self, in_channels, n_layers=5, input_size=512, norm=False, pool=False):
        ...
        super().__init__(
            *layers,
            Rearrange('b z h w -> b (z h w)'),
            nn.Linear(prev_filt * input_size * input_size, 1)
        )

        
class ConditionalDiscriminator(nn.Module):
    """
    Discriminator conditioned on a continuous Zoobot feature vector
    instead of a discrete learned embedding.
    
    zoobot_dim : int  — dimensionality of the Zoobot vector (e.g. 1280)
    cond_features : int — how many dims to project it down to before concat
    """
    def __init__(self, in_channels=1, zoobot_dim=1280,
                 cond_features=128, norm=False):
        super().__init__()

        # Project Zoobot vector → cond_features
        # Two-layer MLP so it can learn a nonlinear compression
        self.cond_proj = nn.Sequential(
            nn.Linear(zoobot_dim, 256),
            nn.LeakyReLU(0.2),
            nn.Linear(256, cond_features),
            nn.LeakyReLU(0.2),
        )

        self.Down_0 = DownConvLayer(in_channels, 16, 'leakyrelu', kernel_size=21, norm=norm, iteration=0)
        self.Down_1 = DownConvLayer(16, 32,  'leakyrelu', kernel_size=21, norm=norm, iteration=1)
        self.Down_2 = DownConvLayer(32, 64,  'leakyrelu', kernel_size=21, norm=norm, iteration=2)
        self.Down_3 = DownConvLayer(64, 128, 'leakyrelu', kernel_size=21, norm=norm, iteration=3)
        self.Down_4 = DownConvLayer(128, 256,'leakyrelu', kernel_size=21, norm=norm, iteration=4)
        # 256 → 128 → 64 → 32 → 16 → 8  (5 strides of 2 from input 256)

        self.rearr_final = Rearrange('b z h w -> b (z h w)')
        # 256 channels * 8 * 8 spatial = 16384, plus cond_features
        self.act_final = nn.Linear(256 * 8 * 8 + cond_features, 1)

    def forward(self, x, zoobot_vec):
        x = self.Down_0(x)
        x = self.Down_1(x)
        x = self.Down_2(x)
        x = self.Down_3(x)
        ...
        cond  = self.cond_proj(zoobot_vec)
        merged = torch.cat([feats, cond], dim=1)
        return self.act_final(merged)
...
#         # 5 downsamples: 512 -> 256 -> 128 -> 64 -> 32 -> 16
#         prev_filt = 8

#         self.Down_0 = DownConvLayer(
#             in_channels, 16, activation='leakyrelu',
#             kernel_size=21,      # <-- ONLY CHANGE
#             norm=norm, iteration=0
#         )
#         self.Down_1 = DownConvLayer(
#             16, 32, activation='leakyrelu',
...
#         self.rearr_final = Rearrange('b z h w -> b (z h w)')
#         self.act_final = nn.Linear(prev_filt * input_size * input_size, 1)
...
class DiscriminatorFeatures(nn.Module):
    def __init__(self, in_channels, n_layers=6, input_size=512, norm=False, pool=False):
        super().__init__()

        prev_filt = 8
        layers = []

        # 6 downsamples: 512 -> 256 -> 128 -> 64 -> 32 -> 16 -> 8
        for i in range(n_layers):
            layers.append(
                DownConvLayer(
                    prev_filt if i > 0 else in_channels,
        ...
        self.down_layers = nn.Sequential(*layers)

        # final spatial size = 8 x 8
        flat_dim = prev_filt * input_size * input_size

        self.rearr_final = Rearrange('b z h w -> b (z h w)')
        self.act_final = nn.Linear(flat_dim, 1)

    def forward(self, x):
        x = self.down_layers(x)
        feats = self.rearr_final(x)
        out = self.act_final(feats)
        return out, feats
    ...
    def __init__(self, in_channels, n_z=128, n_layers=5, input_size=512, norm=False, pool=False):
        ...
        for i in range(n_layers):
            modules[f'Down_{i}'] = DownConvLayer(
                ...
                prev_filt * 2,
                activation='leakyrelu',
                kernel_size=21,   # <-- ONLY CHANGE NEEDED
                norm=False,
                iteration=i
            )
            prev_filt = prev_filt * 2
...
class ConditionalDiscriminator(nn.Module):
    ...
    def forward(self, x, zoobot_vec, return_features=False):
        # Run the image through the conv stack
        x = self.Down_0(x)
        x = self.Down_1(x)
        x = self.Down_2(x)
        x = self.Down_3(x)
        ...
        # This is the rich feature vector — same role as in DiscriminatorFeatures
        img_feats = self.rearr_final(x)          # (B, 256*8*8)
        
        # Project and concat Zoobot condition
        cond = self.cond_proj(zoobot_vec)         # (B, cond_features)
        merged = torch.cat([img_feats, cond], dim=1)
        
        scalar = self.act_final(merged)           # (B, 1)

        if return_features:
            # Return img_feats — NOT merged, because merged includes the 
            # Zoobot condition which you don't want in the reconstruction loss.
            # img_feats is the pure image representation, which is what you
            # want to match between real and reconstructed images.
            return scalar, img_feats
...
class ConditionalGenerator(nn.Module):
    """
    Generator conditioned on a continuous Zoobot feature vector.
    
    zoobot_dim : int  — dimensionality of the Zoobot vector (e.g. 1280)
    cond_features : int — how many dims to project it down to before concat with z
    """
    def __init__(self, n_z, zoobot_dim=1280, cond_features=128,
                 input_filt=512, final_size=256,
                 out_channels=3, norm=False, pool=False):
        super().__init__()
        self.n_z = n_z
        ...
        # Project (z + cond) jointly into the spatial seed
        # initial spatial size for 256 output with 5 upsamples = 256/32 = 8
        self.initial_size = final_size // (2 ** 5)   # = 8
        self.input_filt = input_filt

        self.z_proj = nn.Linear(
            n_z + cond_features,
            self.initial_size * self.initial_size * input_filt
        )
        self.act_z = nn.LeakyReLU(0.2, True)
        self.rearrange = Rearrange(
            'b (h w z) -> b z h w',
            h=self.initial_size, w=self.initial_size, z=input_filt
        )

        self.Up_1 = UpConvLayer(input_filt,       input_filt // 2, 'leakyrelu', kernel_size=21, norm=norm, pool=pool)
        self.Up_2 = UpConvLayer(input_filt // 2,  input_filt // 4, 'leakyrelu', kernel_size=21, norm=norm, pool=pool)
        self.Up_3 = UpConvLayer(input_filt // 4,  input_filt // 8, 'leakyrelu', kernel_size=21, norm=norm, pool=pool)
        self.Up_4 = UpConvLayer(input_filt // 8,  input_filt // 16,'leakyrelu', kernel_size=21, norm=norm, pool=pool)
        self.Up_5 = UpConvLayer(input_filt // 16, input_filt // 32,'leakyrelu', kernel_size=21, norm=norm, pool=pool)
        # 8 → 16 → 32 → 64 → 128 → 256

        final_ch = input_filt // 32   # = 16 for input_filt=512
        self.conv_final = nn.Conv2d(final_ch, out_channels, kernel_size=5, padding=2)
        self.act_final = nn.Sigmoid()

    def forward(self, z, zoobot_vec):
        cond = self.cond_proj(zoobot_vec)
        merged = torch.cat([z, cond], dim=1)          # (B, n_z + cond_features)
        out = self.act_z(self.z_proj(merged))
