import torch
import os
import tqdm
import numpy as np
import glob
from collections import defaultdict
from torch import optim
from torch.optim.lr_scheduler import ExponentialLR
from .losses import wasserstein_distance, gradient_penalty
from einops import rearrange
from torch.cuda.amp import autocast, GradScaler


class Trainer:
    """
    Trainer module which contains both the full training driver
    and the batch-level training logic (AMP-enabled version).
    """

    gradient_weight = 10
    neptune_config = None

    def __init__(self, generator, discriminator, savefolder, device='cuda'):
        generator.apply(weights_init)
        discriminator.apply(weights_init)

        self.generator = generator
        self.discriminator = discriminator
        self.device = device

        if savefolder[-1] != '/':
            savefolder += '/'
        self.savefolder = savefolder
        if not os.path.exists(savefolder):
            os.mkdir(savefolder)

        self.start = 1

        # AMP scalers
        self.scaler_g = GradScaler()
        self.scaler_d = GradScaler()
        
    def save_epoch_loss(self, epoch, gloss, dloss):
        filename = os.path.join(
            self.savefolder,
            f"losses_epoch_{epoch:04d}.npy"
        )
        np.save(filename, np.array([gloss, dloss], dtype=object))
        print(f"[Saved epoch loss] {filename}")


    # ------------------------------------------------------------
    # AMP‑ENABLED BATCH FUNCTION
    # ------------------------------------------------------------
    def batch(self, x, train=False):
        """
        Train the generator and discriminator on a single batch (AMP version)
        """

        # Move input to device
        if not isinstance(x, torch.Tensor):
            input_tensor = torch.as_tensor(x, dtype=torch.float).to(self.device)
        else:
            input_tensor = x.to(self.device, non_blocking=True)

        # -------------------------
        # 1. GENERATOR FORWARD
        # -------------------------
        z_samp = torch.randn((input_tensor.shape[0], self.generator.n_z), device=self.device)

        with autocast():
            x_gen = self.generator(z_samp)
            disc_fake = self.discriminator(x_gen)
            gen_loss = -torch.mean(disc_fake)

        if train:
            self.generator.zero_grad(set_to_none=True)
            self.scaler_g.scale(gen_loss).backward()
            self.scaler_g.step(self.gen_optimizer)
            self.scaler_g.update()

        # -------------------------
        # 2. DISCRIMINATOR FORWARD
        # -------------------------
        with autocast():
            disc_real = self.discriminator(input_tensor)
            disc_fake = self.discriminator(x_gen.detach())

            w_dist = wasserstein_distance(disc_real, disc_fake)
            d_regularizer = gradient_penalty(
                self.discriminator, input_tensor, x_gen
            ) * self.gradient_weight

            disc_loss = w_dist + d_regularizer

        if train:
            self.discriminator.zero_grad(set_to_none=True)
            self.scaler_d.scale(disc_loss).backward()
            self.scaler_d.step(self.disc_optimizer)
            self.scaler_d.update()

        # -------------------------
        # 3. RETURN LOSS DICT
        # -------------------------
        keys = ['gen', 'disc', 'w_dist', 'grad_penalty']
        values = [
            gen_loss.item(),
            disc_loss.item(),
            w_dist.item(),
            d_regularizer.item()
        ]

        return {k: v for k, v in zip(keys, values)}

    # ------------------------------------------------------------
    # TRAINING LOOP (unchanged except for AMP scalers)
    # ------------------------------------------------------------
    def train(self, train_data, val_data, epochs,
              dsc_learning_rate=1.e-3,
              gen_learning_rate=1.e-3,
              save_freq=10,
              lr_decay=None,
              decay_freq=5):

        # Learning rate setup
        if lr_decay is not None:
            gen_lr = gen_learning_rate * (lr_decay)**((self.start - 1) / decay_freq)
            dsc_lr = dsc_learning_rate * (lr_decay)**((self.start - 1) / decay_freq)
        else:
            gen_lr = gen_learning_rate
            dsc_lr = dsc_learning_rate

        # Neptune logging
        if self.neptune_config is not None:
            self.neptune_config['model/parameters/gen_learning_rate'] = gen_lr
            self.neptune_config['model/parameters/dsc_learning_rate'] = dsc_lr
            self.neptune_config['model/parameters/start'] = self.start
            self.neptune_config['model/parameters/n_epochs'] = epochs

        # Optimizers
        self.gen_optimizer = optim.NAdam(
            self.generator.parameters(), lr=gen_lr, betas=(0.5, 0.9))
        self.disc_optimizer = optim.NAdam(
            self.discriminator.parameters(), lr=dsc_lr, betas=(0.5, 0.9))

        # LR schedulers
        if lr_decay is not None:
            gen_scheduler = ExponentialLR(self.gen_optimizer, gamma=lr_decay)
            dsc_scheduler = ExponentialLR(self.disc_optimizer, gamma=lr_decay)
        else:
            gen_scheduler = None
            dsc_scheduler = None

        # Epoch loss history
        D_loss_ep, G_loss_ep = [], []

        # ------------------------------------------------------------
        # MAIN TRAINING LOOP
        # ------------------------------------------------------------
        for epoch in range(self.start, epochs + 1):

            if isinstance(gen_scheduler, ExponentialLR):
                gen_lr = gen_scheduler.get_last_lr()[0]
                dsc_lr = dsc_scheduler.get_last_lr()[0]

            print(f"Epoch {epoch} -- lr: {gen_lr:5.3e}, {dsc_lr:5.3e}")
            print("-------------------------------------------------------")

            # TRAINING
            self.generator.train()
            self.discriminator.train()

            pbar = tqdm.tqdm(train_data, desc='Training: ', dynamic_ncols=True)
            losses = defaultdict(list)

            for i, input_img in enumerate(pbar):
                if isinstance(input_img, (list, tuple)):
                    input_img = input_img[0]

                batch_loss = self.batch(input_img, train=True)

                # Track losses
                loss_mean = {}
                for key, value in batch_loss.items():
                    losses[key].append(value)
                    loss_mean[key] = np.mean(losses[key])

                pbar.set_postfix_str(
                    " ".join([f"{k}: {v:.2e}" for k, v in loss_mean.items()])
                )

            D_loss_ep.append(loss_mean['disc'])
            G_loss_ep.append(loss_mean['gen'])
            
            # NEW: save loss for this epoch immediately
            self.save_epoch_loss(epoch, G_loss_ep[-1], D_loss_ep[-1])


            # VALIDATION
            self.generator.eval()
            self.discriminator.eval()

            pbar = tqdm.tqdm(val_data, desc='Validation: ')
            losses = defaultdict(list)

            for i, input_img in enumerate(pbar):
                if isinstance(input_img, (list, tuple)):
                    input_img = input_img[0]

                batch_loss = self.batch(input_img, train=False)

                loss_mean = {}
                for key, value in batch_loss.items():
                    losses[key].append(value)
                    loss_mean[key] = np.mean(losses[key])

                pbar.set_postfix_str(
                    " ".join([f"{k}: {v:.2e}" for k, v in loss_mean.items()])
                )

            # LR decay
            if gen_scheduler is not None and dsc_scheduler is not None:
                if epoch % decay_freq == 0:
                    gen_scheduler.step()
                    dsc_scheduler.step()

            # Save checkpoints
            if epoch % save_freq == 0:
                self.save(epoch)
                self.save_metrics(G_loss_ep, D_loss_ep)

        return G_loss_ep, D_loss_ep

    # ------------------------------------------------------------
    # CHECKPOINTING
    # ------------------------------------------------------------
    def save(self, epoch):
        gen_savefile = f'{self.savefolder}/generator_ep_{epoch:03d}.pth'
        disc_savefile = f'{self.savefolder}/discriminator_ep_{epoch:03d}.pth'
        print(f"Saving to {gen_savefile} and {disc_savefile}")
        torch.save(self.generator.state_dict(), gen_savefile)
        torch.save(self.discriminator.state_dict(), disc_savefile)

    def save_metrics(self, gloss, dloss):
        np.save(f'{self.savefolder}/loss_values.npy', np.array([gloss, dloss]))

    def load_last_checkpoint(self):
        gen_checkpoints = sorted(glob.glob(self.savefolder + "generator_ep*.pth"))
        disc_checkpoints = sorted(glob.glob(self.savefolder + "discriminator_ep*.pth"))

        gen_epochs = set([int(ch.split('/')[-1].replace('generator_ep_', '')[:-4]) for ch in gen_checkpoints])
        dsc_epochs = set([int(ch.split('/')[-1].replace('discriminator_ep_', '')[:-4]) for ch in disc_checkpoints])

        try:
            assert len(gen_epochs) > 0, "No checkpoints found!"
            start = max(gen_epochs.union(dsc_epochs))
            self.load(f"{self.savefolder}/generator_ep_{start:03d}.pth",
                      f"{self.savefolder}/discriminator_ep_{start:03d}.pth")
            self.start = start + 1
        except Exception as e:
            print(e)
            print("Checkpoints not loaded")

    def load(self, generator_save, discriminator_save):
        print(generator_save, discriminator_save)
        self.generator.load_state_dict(torch.load(generator_save))
        self.discriminator.load_state_dict(torch.load(discriminator_save))
        print(f"Loaded checkpoints from {generator_save} and {discriminator_save}")


class ConditionalTrainer:
    """
    Trainer for the Zoobot-conditioned WGAN.
    Expects DataLoader to yield (image, zoobot_vec) pairs.
    """

    gradient_weight = 10
    neptune_config = None

    def __init__(self, generator, discriminator, savefolder, device='cuda'):
        # generator  : ConditionalGenerator
        # discriminator : ConditionalDiscriminator
        generator.apply(weights_init)
        discriminator.apply(weights_init)

        self.generator = generator
        self.discriminator = discriminator
        self.device = device

        if savefolder[-1] != '/':
            savefolder += '/'
        self.savefolder = savefolder
        if not os.path.exists(savefolder):
            os.mkdir(savefolder)

        self.start = 1
        self.scaler_g = GradScaler()
        self.scaler_d = GradScaler()

    # ------------------------------------------------------------------
    def batch(self, x, zoobot_vec, train=False):
        """
        Single batch update.
        x           : real images  (B, C, H, W)
        zoobot_vec  : Zoobot vectors for those images  (B, zoobot_dim)
        """
        if not isinstance(x, torch.Tensor):
            input_tensor = torch.as_tensor(x, dtype=torch.float).to(self.device)
        else:
            input_tensor = x.to(self.device, non_blocking=True)

        zoobot_vec = zoobot_vec.to(self.device, non_blocking=True)

        # ---- GENERATOR ----
        z_samp = torch.randn(
            (input_tensor.shape[0], self.generator.n_z), device=self.device
        )

        with autocast():
            x_gen = self.generator(z_samp, zoobot_vec)
            # Discriminator judges fake images *with the same Zoobot condition*
            disc_fake = self.discriminator(x_gen, zoobot_vec)
            gen_loss = -torch.mean(disc_fake)

        if train:
            self.generator.zero_grad(set_to_none=True)
            self.scaler_g.scale(gen_loss).backward()
            self.scaler_g.step(self.gen_optimizer)
            self.scaler_g.update()

        # ---- DISCRIMINATOR ----
        with autocast():
            disc_real = self.discriminator(input_tensor, zoobot_vec)
            disc_fake = self.discriminator(x_gen.detach(), zoobot_vec)

            w_dist = wasserstein_distance(disc_real, disc_fake)
            # gradient penalty needs to know the condition too
            d_regularizer = self._gradient_penalty(
                input_tensor, x_gen.detach(), zoobot_vec
            ) * self.gradient_weight

            disc_loss = w_dist + d_regularizer

        if train:
            self.discriminator.zero_grad(set_to_none=True)
            self.scaler_d.scale(disc_loss).backward()
            self.scaler_d.step(self.disc_optimizer)
            self.scaler_d.update()

        return {
            'gen':         gen_loss.item(),
            'disc':        disc_loss.item(),
            'w_dist':      w_dist.item(),
            'grad_penalty': d_regularizer.item(),
        }

    # ------------------------------------------------------------------
    def _gradient_penalty(self, real, fake, zoobot_vec):
        """
        WGAN-GP gradient penalty — conditional version.
        The discriminator input is (interpolated image, zoobot_vec).
        """
        B = real.size(0)
        alpha = torch.rand(B, 1, 1, 1, device=self.device)
        interpolated = (alpha * real + (1 - alpha) * fake).requires_grad_(True)

        d_interp = self.discriminator(interpolated, zoobot_vec)
        gradients = torch.autograd.grad(
            outputs=d_interp,
            inputs=interpolated,
            grad_outputs=torch.ones_like(d_interp),
            create_graph=True,
            retain_graph=True,
        )[0]

        gradients = gradients.view(B, -1)
        penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean()
        return penalty

    # ------------------------------------------------------------------
    def train(self, train_data, val_data, epochs,
              dsc_learning_rate=1e-3, gen_learning_rate=1e-3,
              save_freq=10, lr_decay=None, decay_freq=5):

        if lr_decay is not None:
            gen_lr = gen_learning_rate * (lr_decay) ** ((self.start - 1) / decay_freq)
            dsc_lr = dsc_learning_rate * (lr_decay) ** ((self.start - 1) / decay_freq)
        else:
            gen_lr = gen_learning_rate
            dsc_lr = dsc_learning_rate

        self.gen_optimizer = optim.NAdam(
            self.generator.parameters(), lr=gen_lr, betas=(0.5, 0.9))
        self.disc_optimizer = optim.NAdam(
            self.discriminator.parameters(), lr=dsc_lr, betas=(0.5, 0.9))

        if lr_decay is not None:
            gen_scheduler = ExponentialLR(self.gen_optimizer, gamma=lr_decay)
            dsc_scheduler = ExponentialLR(self.disc_optimizer, gamma=lr_decay)
        else:
            gen_scheduler = dsc_scheduler = None

        D_loss_ep, G_loss_ep = [], []

        for epoch in range(self.start, epochs + 1):
            print(f"Epoch {epoch} -- gen_lr: {gen_lr:.3e}  dsc_lr: {dsc_lr:.3e}")
            print("-------------------------------------------------------")

            # ---- TRAINING ----
            self.generator.train()
            self.discriminator.train()

            pbar = tqdm.tqdm(train_data, desc='Training', dynamic_ncols=True)
            losses = defaultdict(list)

            for batch_data in pbar:
                # DataLoader yields (image, zoobot_vec)
                input_img, zoobot_vec = batch_data
                batch_loss = self.batch(input_img, zoobot_vec, train=True)

                loss_mean = {}
                for key, value in batch_loss.items():
                    losses[key].append(value)
                    loss_mean[key] = np.mean(losses[key])

                pbar.set_postfix_str(
                    " ".join([f"{k}: {v:.2e}" for k, v in loss_mean.items()])
                )

            D_loss_ep.append(loss_mean['disc'])
            G_loss_ep.append(loss_mean['gen'])
            self.save_epoch_loss(epoch, G_loss_ep[-1], D_loss_ep[-1])

            # ---- VALIDATION ----
            self.generator.eval()
            self.discriminator.eval()

            pbar = tqdm.tqdm(val_data, desc='Validation')
            losses = defaultdict(list)

            for batch_data in pbar:
                input_img, zoobot_vec = batch_data
                batch_loss = self.batch(input_img, zoobot_vec, train=False)

                loss_mean = {}
                for key, value in batch_loss.items():
                    losses[key].append(value)
                    loss_mean[key] = np.mean(losses[key])

                pbar.set_postfix_str(
                    " ".join([f"{k}: {v:.2e}" for k, v in loss_mean.items()])
                )

            if gen_scheduler is not None:
                if epoch % decay_freq == 0:
                    gen_scheduler.step()
                    dsc_scheduler.step()

            if epoch % save_freq == 0:
                self.save(epoch)
                self.save_metrics(G_loss_ep, D_loss_ep)

        return G_loss_ep, D_loss_ep

    # ------------------------------------------------------------------
    def save_epoch_loss(self, epoch, gloss, dloss):
        fname = os.path.join(self.savefolder, f"losses_epoch_{epoch:04d}.npy")
        np.save(fname, np.array([gloss, dloss], dtype=object))

    def save(self, epoch):
        gen_f  = f'{self.savefolder}/cond_generator_ep_{epoch:03d}.pth'
        disc_f = f'{self.savefolder}/cond_discriminator_ep_{epoch:03d}.pth'
        torch.save(self.generator.state_dict(), gen_f)
        torch.save(self.discriminator.state_dict(), disc_f)
        print(f"Saved to {gen_f} and {disc_f}")

    def save_metrics(self, gloss, dloss):
        np.save(f'{self.savefolder}/loss_values.npy', np.array([gloss, dloss]))

    def load_last_checkpoint(self):
        gen_checkpoints = sorted(
            glob.glob(self.savefolder + "cond_generator_ep*.pth"))
        disc_checkpoints = sorted(
            glob.glob(self.savefolder + "cond_discriminator_ep*.pth"))

        gen_epochs = set([
            int(ch.split('/')[-1].replace('cond_generator_ep_', '')[:-4])
            for ch in gen_checkpoints
        ])
        dsc_epochs = set([
            int(ch.split('/')[-1].replace('cond_discriminator_ep_', '')[:-4])
            for ch in disc_checkpoints
        ])

        try:
            assert len(gen_epochs) > 0, "No checkpoints found!"
            start = max(gen_epochs.union(dsc_epochs))
            self.load(
                f"{self.savefolder}/cond_generator_ep_{start:03d}.pth",
                f"{self.savefolder}/cond_discriminator_ep_{start:03d}.pth"
            )
            self.start = start + 1
        except Exception as e:
            print(e)
            print("Checkpoints not loaded")

    def load(self, generator_save, discriminator_save):
        self.generator.load_state_dict(torch.load(generator_save))
        self.discriminator.load_state_dict(torch.load(discriminator_save))
        print(f"Loaded from {generator_save} and {discriminator_save}")

# class Trainer:
#     '''
#         Trainer module which contains both the full training driver
#         which calls the train_batch method
#     '''

#     gradient_weight = 10

#     neptune_config = None

#     def __init__(self, generator, discriminator, savefolder, device='cuda'):
#         '''
#             Store the generator and discriminator info
#         '''

#         generator.apply(weights_init)
#         discriminator.apply(weights_init)

#         self.generator = generator
#         self.discriminator = discriminator
#         self.device = device

#         if savefolder[-1] != '/':
#             savefolder += '/'

#         self.savefolder = savefolder
#         if not os.path.exists(savefolder):
#             os.mkdir(savefolder)

#         self.start = 1

#     def batch(self, x, train=False):
#         '''
#             Train the generator and discriminator on a single batch
#         '''

#         if not isinstance(x, torch.Tensor):
#             input_tensor = torch.as_tensor(x, dtype=torch.float).to(self.device)
#         else:
#             input_tensor = x.to(self.device, non_blocking=True)

# #         input_tensor = rearrange(input_tensor, 'b t c x y -> (b t) c x y')
        
#         z_samp = torch.randn((input_tensor.shape[0], self.generator.n_z), device=self.device)
#         x_gen = self.generator(z_samp)

#         disc_fake = self.discriminator(x_gen)
#         gen_loss = -torch.mean(disc_fake)

#         if train:
#             self.generator.zero_grad()
#             gen_loss.backward()
#             self.gen_optimizer.step()

#         disc_real = self.discriminator(input_tensor)

#         disc_fake = self.discriminator(x_gen.detach())
#         w_dist = wasserstein_distance(disc_real, disc_fake)
# #         print(input_tensor.shape, x_gen.shape)
#         d_regularizer = gradient_penalty(self.discriminator, input_tensor, x_gen) * self.gradient_weight

#         disc_loss = w_dist + d_regularizer

#         if train:
#             self.discriminator.zero_grad()
#             disc_loss.backward()
#             self.disc_optimizer.step()

#         keys = ['gen', 'disc', 'w_dist', 'grad_penalty']
#         mean_loss_i = [gen_loss.item(), disc_loss.item(), w_dist.item(), d_regularizer.item()]

#         loss = {key: val for key, val in zip(keys, mean_loss_i)}

#         return loss

#     def train(self, train_data, val_data, epochs, dsc_learning_rate=1.e-3,
#               gen_learning_rate=1.e-3, save_freq=10, lr_decay=None, decay_freq=5):
#         '''
#             Training driver which loads the optimizer and calls the
#             `train_batch` method. Also handles checkpoint saving
#             Inputs
#             ------
#             train_data : DataLoader object
#                 Training data that is mapped using the DataLoader or
#                 MmapDataLoader object defined in io.py
#             val_data : DataLoader object
#                 Validation data loaded in using the DataLoader or
#                 MmapDataLoader object
#             epochs : int
#                 Number of epochs to run the model
#             dsc_learning_rate : float [default: 1e-4]
#                 Initial learning rate for the discriminator
#             gen_learning_rate : float [default: 1e-3]
#                 Initial learning rate for the generator
#             save_freq : int [default: 10]
#                 Frequency at which to save checkpoints to the save folder
#             lr_decay : float [default: None]
#                 Learning rate decay rate (ratio of new learning rate
#                 to previous). A value of 0.95, for example, would set the
#                 new LR to 95% of the previous value
#             decay_freq : int [default: 5]
#                 Frequency at which to decay the learning rate. For example,
#                 a value of for decay_freq and 0.95 for lr_decay would decay
#                 the learning to 95% of the current value every 5 epochs.
#             Outputs
#             -------
#             G_loss_plot : numpy.ndarray
#                 Generator loss history as a function of the epochs
#             D_loss_plot : numpy.ndarray
#                 Discriminator loss history as a function of the epochs
#         '''

#         if (lr_decay is not None):
#             gen_lr = gen_learning_rate * (lr_decay)**((self.start - 1) / (decay_freq))
#             dsc_lr = dsc_learning_rate * (lr_decay)**((self.start - 1) / (decay_freq))
#         else:
#             gen_lr = gen_learning_rate
#             dsc_lr = dsc_learning_rate

#         if self.neptune_config is not None:
#             self.neptune_config['model/parameters/gen_learning_rate'] = gen_lr
#             self.neptune_config['model/parameters/dsc_learning_rate'] = dsc_lr
#             self.neptune_config['model/parameters/start'] = self.start
#             self.neptune_config['model/parameters/n_epochs'] = epochs

#         # create the Adam optimzers
# #         self.gen_optimizer = optim.Adagrad(
# #             self.generator.parameters(), lr=gen_lr)
# #         self.disc_optimizer = optim.Adagrad(
# #             self.discriminator.parameters(), lr=dsc_lr)

#         self.gen_optimizer = optim.NAdam(
#             self.generator.parameters(), lr=gen_lr, betas=(0.5, 0.9))
#         self.disc_optimizer = optim.NAdam(
#             self.discriminator.parameters(), lr=dsc_lr, betas=(0.5, 0.9))

#         # set up the learning rate scheduler with exponential lr decay
#         if lr_decay is not None:
#             gen_scheduler = ExponentialLR(self.gen_optimizer, gamma=lr_decay)
#             dsc_scheduler = ExponentialLR(self.disc_optimizer, gamma=lr_decay)
#             if self.neptune_config is not None:
#                 self.neptune_config['model/parameters/scheduler'] = 'ExponentialLR'
#                 self.neptune_config['model/parameters/decay_freq'] = decay_freq
#                 self.neptune_config['model/parameters/lr_decay'] = lr_decay
#         else:
#             gen_scheduler = None
#             dsc_scheduler = None

#         # empty lists for storing epoch loss data
#         D_loss_ep, G_loss_ep = [], []
#         for epoch in range(self.start, epochs + 1):
#             if isinstance(gen_scheduler, ExponentialLR):
#                 gen_lr = gen_scheduler.get_last_lr()[0]
#                 dsc_lr = dsc_scheduler.get_last_lr()[0]
#             else:
#                 gen_lr = gen_learning_rate
#                 dsc_lr = dsc_learning_rate

#             print(f"Epoch {epoch} -- lr: {gen_lr:5.3e}, {dsc_lr:5.3e}")
#             print("-------------------------------------------------------")

#             # batch loss data
#             pbar = tqdm.tqdm(train_data, desc='Training: ', dynamic_ncols=True)

#             if hasattr(train_data, 'shuffle'):
#                 train_data.shuffle()

#             # set to training mode
#             self.generator.train()
#             self.discriminator.train()

#             losses = defaultdict(list)
#             # loop through the training data
#             for i, input_img in enumerate(pbar):
#                 if isinstance(input_img, (list, tuple)):
#                     input_img = input_img[0]

#                 # train on this batch
#                 batch_loss = self.batch(input_img, train=True)

#                 # append the current batch loss
#                 loss_mean = {}
#                 for key, value in batch_loss.items():
#                     losses[key].append(value)
#                     loss_mean[key] = np.mean(losses[key], axis=0)

#                 loss_str = " ".join([f"{key}: {value:.2e}" for key, value in loss_mean.items()])

#                 pbar.set_postfix_str(loss_str)

#             # update the epoch loss
#             D_loss_ep.append(loss_mean['disc'])
#             G_loss_ep.append(loss_mean['gen'])

#             if self.neptune_config is not None:
#                 self.neptune_config['train/gen_loss'].append(loss_mean['gen'])
#                 self.neptune_config['train/disc_loss'].append(loss_mean['disc'])

#             # validate every `validation_freq` epochs
#             self.discriminator.eval()
#             self.generator.eval()
#             pbar = tqdm.tqdm(val_data, desc='Validation: ')

#             if hasattr(val_data, 'shuffle'):
#                 val_data.shuffle()

#             losses = defaultdict(list)
#             # loop through the training data
#             for i, input_img in enumerate(pbar):
#                 if isinstance(input_img, (list, tuple)):
#                     input_img = input_img[0]

#                 # train on this batch
#                 batch_loss = self.batch(input_img, train=False)

#                 loss_mean = {}
#                 for key, value in batch_loss.items():
#                     losses[key].append(value)
#                     loss_mean[key] = np.mean(losses[key], axis=0)

#                 loss_str = " ".join([f"{key}: {value:.2e}" for key, value in loss_mean.items()])

#                 pbar.set_postfix_str(loss_str)

#             if self.neptune_config is not None:
#                 self.neptune_config['eval/gen_loss'].append(loss_mean['gen'])
#                 self.neptune_config['eval/disc_loss'].append(loss_mean['disc'])

#             # apply learning rate decay
#             if (gen_scheduler is not None) & (dsc_scheduler is not None):
#                 if isinstance(gen_scheduler, ExponentialLR):
#                     if epoch % decay_freq == 0:
#                         gen_scheduler.step()
#                         dsc_scheduler.step()

#             # save checkpoints
#             if epoch % save_freq == 0:
#                 self.save(epoch)
#                 self.save_metrics(G_loss_ep, D_loss_ep)

#         return G_loss_ep, D_loss_ep

#     def save(self, epoch):
#         gen_savefile = f'{self.savefolder}/generator_ep_{epoch:03d}.pth'
#         disc_savefile = f'{self.savefolder}/discriminator_ep_{epoch:03d}.pth'

#         print(f"Saving to {gen_savefile} and {disc_savefile}")
#         torch.save(self.generator.state_dict(), gen_savefile)
#         torch.save(self.discriminator.state_dict(), disc_savefile)
    
#     def save_metrics(self, gloss, dloss):
#         np.save(f'{self.savefolder}/loss_values.npy', np.array([gloss, dloss]))
        
#     def load_last_checkpoint(self):
#         gen_checkpoints = sorted(
#             glob.glob(self.savefolder + "generator_ep*.pth"))
#         disc_checkpoints = sorted(
#             glob.glob(self.savefolder + "discriminator_ep*.pth"))

#         gen_epochs = set([int(ch.split(
#             '/')[-1].replace('generator_ep_', '')[:-4]) for
#             ch in gen_checkpoints])
#         dsc_epochs = set([int(ch.split(
#             '/')[-1].replace('discriminator_ep_', '')[:-4]) for
#             ch in disc_checkpoints])

#         try:
#             assert len(gen_epochs) > 0, "No checkpoints found!"

#             start = max(gen_epochs.union(dsc_epochs))
#             self.load(f"{self.savefolder}/generator_ep_{start:03d}.pth",
#                       f"{self.savefolder}/discriminator_ep_{start:03d}.pth")
#             self.start = start + 1
#         except Exception as e:
#             print(e)
#             print("Checkpoints not loaded")

#     def load(self, generator_save, discriminator_save):
#         print(generator_save, discriminator_save)
#         self.generator.load_state_dict(torch.load(generator_save))
#         self.discriminator.load_state_dict(torch.load(discriminator_save))

#         gfname = generator_save.split('/')[-1]
#         dfname = discriminator_save.split('/')[-1]
#         print(
#             f"Loaded checkpoints from {gfname} and {dfname}")


def weights_init(net):
    """Initialize network weights.
    Parameters:
        net (network)   -- network to be initialized
    We use 'normal' in the original pix2pix and CycleGAN paper. But xavier and kaiming might
    work better for some applications. Feel free to try yourself.
    """
    def init_func(m):  # define the initialization function
        classname = m.__class__.__name__
        if hasattr(m, 'weight') and (classname.find('Conv')) != -1:
            torch.nn.init.xavier_uniform_(m.weight.data)
        # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
        elif classname.find('InstanceNorm') != -1:
            torch.nn.init.xavier_uniform_(m.weight.data, 1.0)
            torch.nn.init.constant_(m.bias.data, 0.0)
