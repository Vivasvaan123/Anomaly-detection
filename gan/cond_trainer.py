...
#         input_tensor = rearrange(input_tensor, 'b t c x y -> (b t) c x y')
        ...
        x_gen = self.generator(z_samp, input_condition)

        disc_fake, _ = self.discriminator(x_gen, input_condition)
...
#         print(input_tensor.shape, input_condition.shape)
        disc_real, _ = self.discriminator(input_tensor, input_condition)

        disc_fake, _ = self.discriminator(x_gen.detach(), input_condition)
        w_dist = wasserstein_distance(disc_real, disc_fake)
#         print(input_tensor.shape, x_gen.shape)
        d_regularizer = gradient_penalty(self.discriminator, input_tensor, x_gen, cond=input_condition) * self.gradient_weight
