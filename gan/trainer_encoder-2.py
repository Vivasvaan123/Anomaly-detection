...
class EncoderTrainer:
    ...
    def batch(self, x, train=False):
        ...
        with autocast():
            ...
            z_enc = self.encoder(input_tensor)
            gen_img = self.generator(z_enc)

            # 2. Discriminator features
            with torch.no_grad():
                _, disc_features_real = self.discriminator(input_tensor)
            _, disc_features_gen = self.discriminator(gen_img)   # outside no_grad

#             img_loss  = torch.sum((input_tensor - gen_img)**2)
#             feat_loss = torch.sum((disc_features_real.detach() - disc_features_gen)**2)
            
            # Shape: (B, C, H, W) → sum over C,H,W → mean over B
            img_loss  = ((input_tensor - gen_img)**2).sum(dim=[1,2,3]).mean()

            # Shape: (B, N_features) → sum over N_features → mean over B  
            feat_loss = (((disc_features_real.detach() - disc_features_gen)**2).sum(dim=1).mean())*10**(7.43)
...
#             z_hat = self.encoder(x_fake)
# #             latent_loss = torch.sum((z_hat - z_rand)**2)
            
#             latent_loss = ((z_hat - z_rand)**2).sum(dim=1).mean()
        ...
        for epoch in range(self.start, epochs + 1):
            ...
            for input_img in pbar:
                ...
                pbar.set_postfix_str(
                    " ".join([f"{k}: {np.mean(v):.2e}" for k, v in losses.items()])
                )

            # Compute epoch means
            epoch_losses = {k: np.mean(v) for k, v in losses.items()}

            # -------------------------
            # SAVE PER-EPOCH LOSS FILE
            # -------------------------
            np.save(
                f"{self.savefolder}/losses_epoch_{epoch:04d}.npy",
                np.array([epoch_losses['img_loss'],
                          epoch_losses['feat_loss'],
#                           epoch_losses['latent_loss'],
                          epoch_losses['total_loss']], dtype=object)
            )
            ...
            for input_img in pbar:
                ...
                pbar.set_postfix_str(
                    " ".join([f"{k}: {np.mean(v):.2e}" for k, v in val_losses.items()])
                )
...
class ConditionalEncoderTrainer:
    """
    Trains the Encoder to invert the ConditionalGenerator.
    DataLoader must yield (image, zoobot_vec) pairs.
    
    Loss = reconstruction (pixel) + feature matching (discriminator)
           + latent regression
    
    The key difference from EncoderTrainer:
      - generator.forward(z, zoobot_vec) requires the Zoobot vector
      - the Zoobot vector is passed through from the dataloader
      - for anomaly detection: at test time you can fix zoobot_vec and
        search only over z, making the anomaly score interpretable
    """

    def __init__(self, encoder, generator, discriminator,
                 savefolder, device='cuda'):
        encoder.apply(weights_init)
        ...
        self.start = 1
        self.alpha = 0.5   # weight between pixel loss and feature loss
        self.scaler = GradScaler()

    # ------------------------------------------------------------------
    def batch(self, x, zoobot_vec, train=False):
        input_tensor = x.to(self.device, non_blocking=True) if isinstance(x, torch.Tensor) \
                       else torch.as_tensor(x, dtype=torch.float).to(self.device)
        zoobot_vec = zoobot_vec.to(self.device, non_blocking=True)
        B = input_tensor.size(0)
        n_z = self.encoder.n_z

        with autocast():
            # 1. Encode real image → z, reconstruct with known Zoobot condition
            z_enc = self.encoder(input_tensor)
            gen_img = self.generator(z_enc, zoobot_vec)

            # 2. Feature matching — ask discriminator for features, not scalar
            with torch.no_grad():
                _, feats_real = self.discriminator(input_tensor, zoobot_vec,
                                                   return_features=True)
            _, feats_gen = self.discriminator(gen_img, zoobot_vec,
                                              return_features=True)

            img_loss  = ((input_tensor - gen_img)**2).sum(dim=[1,2,3]).mean()
            feat_loss = (((feats_real.detach() - feats_gen)**2).sum(dim=1).mean())*10**(7.43)

            # 3. Latent regression
            with torch.no_grad():
                z_rand = torch.randn(B, n_z, device=self.device)
                x_fake = self.generator(z_rand, zoobot_vec)
...
#             'latent_loss': latent_loss.item(),
    ...
    def train(self, train_data, val_data, epochs, lr=1e-4, save_freq=10):
        ...
        self.optimizer = optim.NAdam(
            self.encoder.parameters(), lr=lr, betas=(0.5, 0.9))
        ...
        for epoch in range(self.start, epochs + 1):
            ...
            np.save(
                f"{self.savefolder}/losses_epoch_{epoch:04d}.npy",
                np.array(list(epoch_losses.values()), dtype=object)
            )
    ...
    # ------------------------------------------------------------------
    def save(self, epoch):
        fname = f'{self.savefolder}/cond_encoder_ep_{epoch:03d}.pth'
